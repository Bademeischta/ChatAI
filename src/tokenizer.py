...
class Tokenizer:
    ...
    def save_vocab(self, path: str) -> None:
        """Persist the current vocabulary as JSON."""
        import json
    ...
    def load_vocab(self, path: str) -> None:
        """Load vocabulary from ``path`` written by :func:`save_vocab`."""
        import json

        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
        self.token_to_id = {k: int(v) if isinstance(v, str) and v.isdigit() else v for k, v in data["token_to_id"].items()}
